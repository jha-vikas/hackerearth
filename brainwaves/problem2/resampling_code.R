...
#d_train[348979,cat_variables] <- paste0("Unk_cat_var_", c(1:30,32:34,39,41))
#na_names <- names(d_train)[is.na(d_train[348979,])]
#d_train[348979,na_names] <- d_train[xx,na_names]
...
#based on prop check
d_test$cat_var_1[d_test$cat_var_1 == "yw"]<- "ye" #based on prop check
d_test$cat_var_1[d_test$cat_var_1 == "gz"]<- "ex" #based on prop check
d_test$cat_var_1[d_test$cat_var_1 == "jk"]<- "ln" #based on prop check

d_test$cat_var_3[d_test$cat_var_3 == "ex"]<- "zl" #based on prop check
d_test$cat_var_3[d_test$cat_var_3 == "ye"]<- "db" #based on prop check
d_test$cat_var_3[d_test$cat_var_3 == "hr"]<- "ap" #based on prop check
d_test$cat_var_3[d_test$cat_var_3 == "ap"]<- "Unk_cat_var_3" #based on prop check

d_test$cat_var_2[d_test$cat_var_2 %in% c("op","uq","on")] <- "ce"

d_test$cat_var_7[d_test$cat_var_7 %in% c("cg", "xy", "mm")] <- "ep"

##based on data exploration, the proportional values for categorical variables, imputaiton is done for some categorical variables
d_train$cat_var_8[d_train$cat_var_8 == "Unk_cat_var_8"] <- "da"
d_test$cat_var_8[d_test$cat_var_8 == "Unk_cat_var_8"] <- "da"
d_test$cat_var_8[d_test$cat_var_8 %in% c("db","mh")] <- "dn"


missing_levels <- lapply(cat_variables, FUN = function(x){unique(d_test[[x]])[!unique(d_test[[x]]) %in% unique(d_train[[x]])]})
missing_levels

missing_levels1 <- lapply(cat_variables, FUN = function(x){unique(d_train[[x]])[!unique(d_train[[x]]) %in% unique(d_test[[x]])]})
missing_levels1
...
for(i in 1:length(missing_levels))
{
  ...
  sort(prop.table(table(d_test[[cat_variables[i]]])),decreasing = T)[1:10]
  w <- names(sort(prop.table(table(d_test[[cat_variables[i]]])),decreasing = T)[1:10])
}

# for(i in 1:length(missing_levels1))
# {
#   q <- missing_levels1[[i]]
#   if(length(q)>0)
#   {
...
#     {
#       print(paste(cat_variables[i],q[j],sep = ":" ))
#       print(prop.table(table(d_train[cat_variables[i]] == q[j])))
#       #d_test[cat_variables[i]][d_test[cat_variables[i]] == q[j]] <- paste0("Unk_cat_var_",i)
#     }
#   }
#   sort(prop.table(table(d_train[[cat_variables[i]]])),decreasing = T)[1:10]
#   w1 <- names(sort(prop.table(table(d_train[[cat_variables[i]]])),decreasing = T)[1:10])
# }
...
gbm <- h2o.getModel(sortedGrid@model_ids[[9]])
...
write.csv(test_output, "./output/gbmh2o_base_cv_upsample.csv", row.names = F, quote = F)
###################################################################################################
######################SMOTE#######################################################################
library(DMwR)
set.seed(1103)
smoted_train <- SMOTE(target ~ ., data = d_train[,-1])

d_train_backup <- d_train

d_train <- smoted_train

upSampledTrain <- upSample(x = d_train_backup[,c(2:43)],
                            y = d_train_backup$target,
