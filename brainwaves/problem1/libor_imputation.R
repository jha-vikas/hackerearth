...
lmTune0 <- train(x = trainT[,finalSet[-c(5,8,11,12)]], y = trainT$return,
                 method = "lm",
                 trControl = ctrl, metric = "Rsquared")
lmTune0
summary(lmTune0)

R2(trainV$return, predict(lmTune0, trainV))

plot(trainV$return, predict(lmTune0, trainV))

head(data.frame(obs = trainV$return, pred = predict(lmTune0, trainV)),20)

test_pred <- predict(lmTune0, d_test)
