...
cartTune <- train(x = trainT[,finalSet], y = trainT$return,
                  method = "rpart",
                  tuneLength = 25,
                  trControl = ctrl, metric = "Rsquared")
cartTune

R2(trainV$return, predict(cartTune, trainV))

plot(trainV$return, predict(cartTune, trainV))

head(data.frame(obs = trainV$return, pred = predict(cartTune, trainV)),20)
...
# indx <- createFolds(d_train$return, returnTrain = TRUE)
# ctrl <- trainControl(method = "cv", index = indx)

cGrid <- data.frame(mincriterion = sort(c(.95, seq(.5, .99, length = 20))))

ctreeTune <- train(x = trainT[,finalSet], y = trainT$return,
                   method = "ctree",
                   tuneGrid = cGrid,
                   trControl = ctrl, metric = "Rsquared")
ctreeTune
...
R2(trainV$return, predict(ctreeTune, trainV))
plot(trainV$return, predict(ctreeTune, trainV))


df_test_pred <- data.frame(portfolio_id = d_test$portfolio_id, return = predict(ctreeTune1, d_test), stringsAsFactors = F)
df_test_pred <- rbind(df_test_pred,direct_return)
...
treebagTune <- train(x = trainT[,finalSet], y = trainT$return,
                     method = "treebag",
                     tuneLength = 20,
                     #nbagg = 50,
                     trControl = ctrl,metric = "Rsquared")
...
R2(trainV$return, predict(treebagTune, trainV))

plot(trainV$return, predict(treebagTune, trainV))
...
mtryGrid <- data.frame(mtry = floor(seq(10, ncol(trainT[,finalSet]), length = 10)))

### Tune the model using cross-validation

rfTune <- train(x = trainT[,finalSet], y = trainT$return,
                method = "rf",
                tuneGrid = mtryGrid,
                ntree = 200,
                importance = TRUE,
...
R2(trainV$return, predict(rfTune, trainV))
...
condrfTune <- train(x = trainT[,finalSet], y = trainT$return,
                    method = "cforest",
                    tuneGrid = mtryGrid,
                    controls = cforest_unbiased(ntree = 200),
                    trControl = ctrl, metric = "Rsquared")

condrfTune
plot(condrfTune)

R2(trainV$return, predict(condrfTune, trainV))

plot(trainV$return, predict(condrfTune, trainV))
